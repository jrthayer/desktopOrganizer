...
internal sealed class TrayMenuRenderer : ToolStripRenderer
{
    ...
    /// <summary>A hand-drawn checkbox+check instead of the default renderer's own glyph - like every
    /// other themed checkbox in this app (see DropdownMenu.DrawRow), the stock one is a small bitmap
    /// baked in for a light background and reads as a washed-out box on a dark one.</summary>
    protected override void OnRenderItemCheck(ToolStripItemImageRenderEventArgs e)
    {
        var rect = e.ImageRectangle.Width > 0 ? e.ImageRectangle : new Rectangle(4, (e.Item.Height - 12) / 2, 12, 12);

        using (var pen = new Pen(AppTheme.Accent))
            e.Graphics.DrawRectangle(pen, rect);
        using var checkPen = new Pen(AppTheme.Accent, 2);
