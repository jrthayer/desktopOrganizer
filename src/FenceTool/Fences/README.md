...
The original Fence Tool feature: draggable, resizable, translucent "fence"
...
The pure edge-snapping geometry itself (`SnapEngine`) lives outside this
folder, at [`src/FenceTool/Snapping`](../Snapping/SnapEngine.cs) — it has no
dependency on fences or any particular widget type, so any future
draggable/resizable UI element in this app can reuse it the same way
`FenceForm` does.
